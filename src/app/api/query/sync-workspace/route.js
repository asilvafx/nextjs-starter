...
async function postHandler(request) {
  try {
    ...
    for (const appointment of upcomingAppointments) {
      try {
        ...
        const existingTask = tasks.find(task => 
          task.appointmentId === appointment.id && task.type === 'appointment-followup'
        );
    ...
    // Clean agenda items
    const orphanedAgendaItems = agendaItems.filter(item => 
      item.appointmentId && !appointmentIds.includes(item.appointmentId)
    );
    ...
    const orphanedScheduleItems = scheduleItems.filter(item => 
      item.appointmentId && !appointmentIds.includes(item.appointmentId)
    );
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to synchronize workspace' },
      { status: 500 }
    );
  }
}
...
async function getHandler() {
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to get synchronization status' },
      { status: 500 }
    );
  }
}
