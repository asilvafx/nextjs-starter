...
// GET API settings
async function handleGetApiSettings(_request) {
    try {
        ...
        return NextResponse.json({ success: false, error: 'Failed to fetch API settings' }, { status: 500 });
    }
}
...
async function handleCreateApiSettings(request) {
    try {
        ...
        return NextResponse.json({ success: false, error: 'Failed to create API settings' }, { status: 500 });
    }
}
...
async function handleUpdateApiSettings(request) {
    try {
        ...
        if (!data.id) {
            return NextResponse.json({ success: false, error: 'Settings ID is required for updates' }, { status: 400 });
        }

        // Check if settings exist
        const existing = await DBService.read(data.id, 'api_settings');
        if (!existing) {
            return NextResponse.json({ success: false, error: 'API settings not found' }, { status: 404 });
        }
        ...
        if (!updated) {
            return NextResponse.json({ success: false, error: 'Failed to update API settings' }, { status: 500 });
        }
