...
import { CheckCircle, Clock, User, AlertCircle, Plus, Star, Filter, Calendar, Users, Edit, Trash2, Eye, MoreHorizontal, Search } from "lucide-react";
      ...
      try { 
        ...
        if (appointmentsData.length > 0) { 
          const appointmentTasks = appointmentsData
            .filter(apt => apt.status === 'confirmed' || apt.status === 'scheduled' || apt.status === 'pending')
            .map(apt => ({
              id: `apt_task_${apt.id}`,
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <div className="flex gap-2 sm:hidden w-full">
                    <Button variant="outline" size="sm" className="flex-1" onClick={() => handleViewTask(task)}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge className={statusConfig[selectedTask.status]?.color || statusConfig.pending.color}>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge className={priorityConfig[selectedTask.priority] || priorityConfig.medium}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm">{selectedTask.assignedTo || selectedTask.assignee || 'Unassigned'}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Due Date</Label>
                    <p className="text-sm">{selectedTask.dueDate ? new Date(selectedTask.dueDate).toLocaleDateString() : 'No due date'}</p>
                  </div>
                </div>
                {selectedTask.linkedAppointment && (
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Linked Appointment</Label>
                    <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg mt-1">
                      <p className="text-sm font-medium">{selectedTask.linkedAppointment.serviceName}</p>
                      <p className="text-xs text-muted-foreground">
                        {selectedTask.linkedAppointment.customerName} • {selectedTask.linkedAppointment.date} at {selectedTask.linkedAppointment.startTime}
                      </p>
                    </div>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
                  <div>
                    ...
                    <p>{selectedTask.createdAt ? new Date(selectedTask.createdAt).toLocaleDateString() : 'Unknown'}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Updated</Label>
                    <p>{selectedTask.updatedAt ? new Date(selectedTask.updatedAt).toLocaleDateString() : 'Never'}</p>
                  </div>
                </div>
              </div>
              <div className="flex justify-end space-x-2">
