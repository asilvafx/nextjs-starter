// @/app/admin/workspace/page.jsx - Workspace Overview with Synchronized Data
...
export default function WorkspaceOverview() {
  ...
  const calculateStats = (workspaceData) => {
    ...
    const todayAppointments = workspaceData.appointments.filter(apt => 
      apt.date === today && apt.status === 'confirmed'
    ).length;

    const pendingTasks = workspaceData.tasks.filter(task => 
      task.status === 'todo' || task.status === 'in-progress'
    ).length;

    const upcomingEvents = [
      ...workspaceData.agendaItems,
          ...
          {getUpcomingEvents().length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {getUpcomingEvents().map((event) => (
                <div key={event.id} className="p-4 bg-muted/50 rounded-lg space-y-2">
