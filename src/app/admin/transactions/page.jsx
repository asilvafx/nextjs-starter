...
export default function TransactionsPage() {
  ...
  const handleSendTransaction = async (e) => {
    ...
    if (numericAmount > numericBalance) {
      toast.error(`Insufficient balance. Available: ${numericBalance.toFixed(6)} ${web3Config?.WEB3_CONTRACT_SYMBOL || 'ETH'}`);
      return;
    }
      ...
      if (result && result.tx_hash) {
        toast.success(
          <div>
            ...
            <div className="text-xs mt-1 font-mono">{result.tx_hash}</div>
          </div>
        );
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'confirmed':
        return <Badge className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300"><CheckCircle className="h-3 w-3 mr-1" />Confirmed</Badge>;
      case 'pending':
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Web3 functionality is currently disabled. Please enable it in System Settings to use wallet features.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{web3Config?.WEB3_NETWORK_NAME}</div>
              <p className="text-xs text-muted-foreground mt-1">
                        ...
                        <TableRow key={tx.id}>
                          ...
                          <TableCell>
                            <span className={tx.type === 'sent' ? 'text-red-500' : 'text-green-500'}>
                              {tx.type === 'sent' ? '-' : '+'}{tx.amount} {web3Config?.WEB3_CONTRACT_SYMBOL || 'ETH'}
                            </span>
                          </TableCell>
                          <TableCell>
                            <code className="text-xs">
                              {formatAddress(tx.type === 'sent' ? tx.toAddress : tx.fromAddress)}
                            </code>
                          </TableCell>
                          <TableCell>
                ...
                <form onSubmit={handleSendTransaction} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>Available: {formattedBalance} {web3Config?.WEB3_CONTRACT_SYMBOL || 'ETH'}</span>
                      {sendForm.amount && (
                        <button 
                          type="button" 
                          onClick={() => setSendForm(prev => ({ ...prev, amount: balance }))} 
              ...
              <CardContent className="space-y-4">
                ...
                <div className="bg-muted p-4 rounded-lg">
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    ...
                    {receiveAmount && (
                      <li>• Expected amount: {receiveAmount} {web3Config?.WEB3_CONTRACT_SYMBOL || 'ETH'}</li>
                    )}
                  </ul>
                </div>
              </CardContent>
