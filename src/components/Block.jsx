...
const Block = ({ slug, className = '', fallback = null, onLoad = null, onError = null }) => {
    ...
    const renderContent = () => {
        switch (block.type) {
            case 'html':
                return <div className="block-html-content" dangerouslySetInnerHTML={{ __html: block.content }} />;
            ...
            case 'form':
                ...
                return <div className="block-form-content" dangerouslySetInnerHTML={{ __html: block.content }} />;

            case 'layout':
                return <div className="block-layout-content" dangerouslySetInnerHTML={{ __html: block.content }} />;

            default:
                return <div className="block-default-content" dangerouslySetInnerHTML={{ __html: block.content }} />;
        }
    };

    return (
        <div className={`block block-${block.type} ${className}`} data-block-slug={slug} data-block-id={block.id}>
            {renderContent()}
        </div>
    );
};
