...
- **Legacy Support**: `/api/payments/eupago` - Redirects to primary API  
- **Service Layer**: `/src/lib/server/eupago.js` - Core business logic
- **Automation**: `/api/cronjobs/check-eupago-payments` - Automated payment checking
...
**GET/POST** `/api/payments/eupago/status`
- Check individual payment status by reference

### Webhook (Future Implementation)

**POST** `/api/payments/eupago/webhook`
- Handle EuPago payment notifications

### Cron Job

**GET/POST** `/api/cronjobs/check-eupago-payments`
...
Set up a cron job to automatically check pending payments:

```bash
# Check every 10 minutes
*/10 * * * * curl -X GET "https://yoursite.com/api/cronjobs/check-eupago-payments?key=your-secret-key"
```
...
- **Main Route**: `/api/payments/eupago/route.js`
- **Status Route**: `/api/payments/eupago/status/route.js`
- **Webhook Route**: `/api/payments/eupago/webhook/route.js`
- **Cron Route**: `/api/cronjobs/check-eupago-payments/route.js`
...
2. **Remove Files**:
   - `src/lib/server/eupago.js`
   - `src/app/api/payments/eupago/` (entire folder)
   - `src/app/api/cronjobs/check-eupago-payments/`
3. **Clean UI Code**:
