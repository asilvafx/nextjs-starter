...
### Before (6 separate HTTP requests):
```
GET /api/query/users?limit=0          - 2489ms
GET /api/query/orders?limit=0         - 2616ms
GET /api/query/catalog?limit=0        - 2010ms
GET /api/query/categories?limit=0     - 2013ms
GET /api/query/collections?limit=0    - 1803ms
GET /api/query/public/site_settings   - 2717ms
---
Total: ~13.6 seconds (sequential)
Network overhead: ~1.2 seconds (6 round trips)
```

### After (1 aggregated HTTP request):
```
GET /api/dashboard/stats - ~2500ms
---
Total: ~2.5 seconds (80% faster!)
Network overhead: ~200ms (1 round trip)
```

### Now (Direct server function):
```
getDashboardStats() - ~2300ms
---
Total: ~2.3 seconds (83% faster!)
